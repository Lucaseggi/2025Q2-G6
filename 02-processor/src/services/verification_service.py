...
class VerificationService(VerificationServiceInterface):
    """Service for verifying LLM responses and quality control"""
    ...
    def verify_structured_response(self, original_text: str, structured_data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            # Check if structured data has expected format
            if not isinstance(structured_data, dict):
                return False, "Structured data is not a dictionary"

            # Validate JSON structure
            is_valid, error_msg = self.validate_json_structure(structured_data)
            if not is_valid:
                return False, f"JSON validation failed: {error_msg}"

            # Calculate similarity score
            similarity_score = self.calculate_similarity_score(original_text, structured_data)

            # Check if similarity is above threshold
            if similarity_score < (1.0 - self.similarity_threshold):
                return False, f"Similarity score {similarity_score:.3f} below threshold"

            return True, "Verification passed"

        except Exception as e:
            self.logger.error(f"Error verifying structured response: {e}")
    ...
    def calculate_similarity_score(self, original_text: str, structured_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            similarity_score = self.calculate_text_similarity(original_text, structured_json)
