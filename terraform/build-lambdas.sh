...
docker run --rm -v "/$(echo $WORK_DIR | sed 's|\\|/|g' | sed 's|:||')/06-relational-guard:/app" -w /app maven:3.9-eclipse-temurin-17 \
...
docker run --rm -v "/$(echo $WORK_DIR | sed 's|\\|/|g' | sed 's|:||')/07-vectorial-guard:/app" -w /app maven:3.9-eclipse-temurin-17 \
  mvn clean package -DskipTests -f pom-lambda.xml
