"""
AWS Lambda handler for purifier service

This is a thin adapter layer that routes between two invocation modes:
1. SQS Trigger: Automatic invocation when messages arrive in purifying queue
...
# Initialize logger
logger = StructuredLogger("purifier", "lambda")

# Initialize services at cold start (reused across warm invocations)
_purifier_service = None


def get_services():
    """Initialize services on cold start, reuse on warm invocations"""
    global _purifier_service

    if _purifier_service is None:
        logger.info("Cold start: Initializing purifier service", stage=LogStage.STARTUP)
        _purifier_service = get_purifier_service()

    return _purifier_service


def handle_sqs_processing(event: Dict[str, Any]) -> Dict[str, Any]:
    """
    Handle SQS trigger: Process messages from purifying queue

    AWS automatically invokes Lambda when messages arrive in SQS queue.
    Processes text through purification pipeline and sends to processing queue.
    ...
    for record in event.get('Records', []):
        ...
        try:
            ...
            # Extract infoleg_id for logging
            ...
            logger.info(
                f"Processing norm {infoleg_id} from SQS",
                stage=LogStage.PROCESSING,
                infoleg_id=infoleg_id
            )

            # Process through purification service
            success, result = purifier_service.process_from_queue(message_body)
            ...
            if success:
                ...
                logger.info(
                    f"Successfully purified norm {infoleg_id}",
                    stage=LogStage.PROCESSING,
                    infoleg_id=infoleg_id
                )
            else:
                ...
                logger.error(
                    f"Failed to purify norm {infoleg_id}: {result}",
                    stage=LogStage.PROCESSING,
                    infoleg_id=infoleg_id
                )
...
def handle_api_replay(event: Dict[str, Any]) -> Dict[str, Any]:
    """
    Handle API Gateway trigger: Replay from cache

    Fetches purified norm from cache and sends to processing queue.
    ...
    try:
        ...
        # Use purifier service
        purifier_service = get_services()
        success, source = purifier_service.replay_norm(infoleg_id, force=force)
        ...
        if success:
            return {
                ...
                'body': json.dumps({
                    'status': 'success',
                    'message': f'Successfully replayed norm {infoleg_id} to processing queue',
                    'infoleg_id': infoleg_id,
                    'source': source
                })
            }
        else:
            status_code = 404 if 'not_found' in source or 'cache_miss' in source else 500
